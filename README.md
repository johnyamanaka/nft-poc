...
2. **資格情報の検証**: Microsoft Authenticatorアプリで資格情報を提示
   - 📱 **QRコード検証**: 対面での資格確認
   - 🔗 **オンライン検証**: リモート面接・オンライン応募向けディープリンク共有
3. **SBTの自動ミント**: 検証成功後、Polygon AmoyネットワークでSBTを自動発行
4. **トランザクション追跡**: PolygonScanでブロックチェーントランザクションを確認
...
**Webフロントエンドの機能**:
- 🎯 **3つの役割ビュー**: 大学（発行）/ 学生（受取）/ 企業（検証）
- 📱 **QRコードの自動生成・表示**: 対面検証用
- 🔗 **ディープリンク共有**: オンライン検証用
  - リンクをクリップボードにコピー
  - メールで送信（定型文テンプレート付き）
- 📊 **サービス状態のリアルタイム監視**: バックエンド・ミントサービス
- 📝 **操作履歴の表示**: アクティビティログ
- ⛓️ **トランザクション追跡**: PolygonScanリンク自動生成
- 🎨 **モダンでレスポンシブなUI**: モバイル対応
...
## 🚀 Phase 1: オンライン検証機能（QRコード不要）

リモート面接やオンライン応募のシナリオに対応するため、QRコードをスキャンせずにディープリンクで検証できる機能を実装しました。

### 実装内容

#### バックエンド
- **検証エンドポイント拡張** (`/api/verify`)
  - `VerificationUrl`フィールドをレスポンスに追加
  - `openid-vc://` プロトコルのディープリンクURLを返却
  - Verified ID APIから取得したURLをそのまま提供

- **ログ機能強化**
  - プレゼンテーションレスポンスの詳細ログ出力
  - デバッグとトラブルシューティングの効率化

#### フロントエンド
- **検証リンク共有UI**
  - 読み取り専用のURL表示ボックス
  - 📋 **リンクをコピー**: Clipboard APIでワンクリックコピー
  - 📧 **メールで送信**: mailto:リンクで定型文付きメール作成
  - 💡 使用方法の説明テキスト
...
- **共有機能の実装** (`script.js`)
  ...
  function sendVerificationEmail() {
      const mailto = `mailto:?subject=...&body=...`;
...
#### 企業側（検証依頼者）
1. Webフロントエンドで「企業」ビューを選択
2. ウォレットアドレスを入力して「検証用QRコードを生成」をクリック
3. 「🔗 オンラインで共有（QRコード不要）」セクションが表示される
4. 以下のいずれかの方法で応募者に共有：
   - **📋 リンクをコピー**: チャット、Slack、Teamsなどで送信
   - **📧 メールで送信**: デフォルトメールアプリが起動、定型文が自動入力

#### 応募者側（検証実施者）
1. 企業から受け取ったリンクをクリック
2. **モバイル**: Microsoft Authenticatorアプリが自動的に起動
3. **PC**: QRコードが表示され、スマートフォンでスキャン
4. 提示する資格情報を選択
5. 「送信」をタップして検証完了

### メリット

✅ **リモートワーク対応**: オンライン面接で資格確認が可能
✅ **ユーザー体験向上**: QRコードスキャンの手間を削減
✅ **柔軟な共有方法**: メール、チャット、SMSなど複数の手段に対応
✅ **セキュリティ維持**: Verified IDの検証フローはそのまま
✅ **追加コスト不要**: 既存のVerified ID機能を活用

### 技術詳細

**ディープリンクURL形式**:
```
openid-vc://?request_uri=https://verifiedid.did.msidentity.com/v1.0/tenants/{tenant-id}/verifiableCredentials/presentationRequests/{request-id}
```

**モバイルOS対応**:
- **iOS**: カスタムURLスキームでAuthenticatorアプリを自動起動
- **Android**: インテント処理でAuthenticatorアプリを自動起動
- **Desktop**: Fallback QRコードを表示
...
### ✅ Phase 1完了（2025年12月）
- ✅ Webフロントエンドの作成（3役割ビュー）
- ✅ リアルタイムステータス表示
- ✅ オンライン検証機能（ディープリンク共有）
- ✅ トランザクション追跡（PolygonScanリンク）
- ✅ 拡張クレーム対応（講座情報、成績、単位数など）
...
**作成日**: 2025年12月10日
**最終更新**: 2025年12月13日
**ステータス**: ✅ PoC完成 + Phase 1実装完了（オンライン検証機能）
